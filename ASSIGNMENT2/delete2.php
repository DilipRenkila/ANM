...
if(isset($DELETE))
{
    ...
    $order2 = "SELECT * FROM INFO2_2 WHERE IP='$ip' and PORT='$port' and COMMUNITY='$community'";
    ...
    if(mysql_num_rows($result) <=0 && mysql_num_rows($result2) <=0)
    ...
    else
       {
              ...
              $order2 = "DELETE FROM INFO2_2 WHERE IP='$ip' and PORT='$port' and COMMUNITY='$community'";
              $result = mysql_query($order);	//order executes
              $result2 = mysql_query($order2);	//order executes
              if($result && $result2)
               {      
